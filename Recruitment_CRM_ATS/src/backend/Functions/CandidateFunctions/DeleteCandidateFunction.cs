...
public class DeleteCandidateFunction
{
    ...
    public DeleteCandidateFunction(
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        _logger = loggerFactory.CreateLogger<DeleteCandidateFunction>();
    }

    [Function("DeleteCandidate")]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "delete", Route = "candidates/{id}")] HttpRequestData req,
        string id)
    {
        _logger.LogInformation("DeleteCandidate function processed a request for {CandidateId}.", id);
        ...
        try
        {
            var deleted = await _databaseService.DeleteCandidateAsync(candidateId);
            ...
            response.WriteString(JsonSerializer.Serialize(new { message = "Candidate deleted successfully" }));

            return response;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting candidate {CandidateId}", candidateId);
